...
- **신뢰도 높은(high-quality)** 계측 데이터를 제공하고
- **시스템 곳곳에서 손쉽게 수집할 수 있고(ubiquitous)**  
- **어떤 환경·벤더에서도 그대로 쓸 수 있도록(portable)** 하여
- **운영자가 서비스 상태를 정확히 들여다볼 수 있도록** 한다
...
```
- **무엇인가요?** 시스템에서 발생하는 이벤트들의 기록
- **언제 사용하나요?** 에러 발생 원인을 찾거나 특정 시점의 상황을 파악할 때
- **예시:** 로그인 실패, 에러 메시지, 시스템 상태 변화 등

### Tracing (추적)
```
사용자 요청 → API Gateway → 인증 서비스 → 데이터베이스 → 응답
     ↓           ↓           ↓           ↓
   50ms       20ms        100ms       30ms
```
- **무엇인가요?** 하나의 요청이 여러 서비스를 거쳐가는 전체 경로와 소요 시간
- **언제 사용하나요?** 어느 구간에서 지연이 발생하는지 찾을 때
...
응답 시간: 평균 120ms
```
- **무엇인가요?** 시스템의 성능과 상태를 숫자로 나타낸 지표
- **언제 사용하나요?** 시스템의 전반적인 건강 상태를 모니터링할 때
- **예시:** CPU/메모리 사용률, 응답 시간, 에러율, 처리량 등

*출처: [mastertheboss.com](https://www.mastertheboss.com/various-stuff/architecture/logging-vs-tracing-vs-metrics-understanding-the-differences/)*
...
참고로 Zero-code 솔루션은 다음과 같은 기술들을 활용한다.

1. **바이트코드 조작 (Bytecode Instrumentation)**
   - JVM 기반 언어에서 런타임에 바이트코드를 수정
   - 예: Java Agent를 통한 자동 계측

2. **몽키 패칭 (Monkey Patching)**  
   - 런타임에 기존 함수나 메서드를 동적으로 수정
   - 예: Python의 동적 함수 교체

3. **eBPF (Extended Berkeley Packet Filter)**
   - 커널 수준에서 애플리케이션 동작 관측
   - 예: 시스템 호출 추적
...
- **벤더 락인 방지**: 모니터링 백엔드 변경 시 계측 코드 재작성 불필요
